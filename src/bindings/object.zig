...
pub fn getObjectValueNum(obj: u32, key: []const u8, comptime T: type) T {
    ...
    switch (@typeInfo(T)) {
        .int => {
            const result: T = @intFromFloat(input);
            return result;
        },
        .float => {
            const result: T = @floatCast(input);
            return result;
        },
        else => {
...
pub fn setObjectValueNum(obj: u32, key: []const u8, comptime T: type, value: T) void {
    ...
    switch (@typeInfo(T)) {
        .int => {
            fValue = @floatFromInt(value);
        },
        .float => {
            fValue = @floatCast(value);
        },
...
pub const Object = struct {
    ...
    pub fn setBool(self: *const Object, key: []const u8, value: bool) void {
        const jsKey = String.new(key);
        defer jsKey.free();
        jsObjectSet(self.id, jsKey.id, if (value) True else common.False);
    }

    pub fn setArray(self: *const Object, key: []const u8, value: *const Array) void {
        const jsKey = String.new(key);
        defer jsKey.free();
        jsObjectSet(self.id, jsKey.id, value.id);
    }
    ...
    pub fn parse(self: *const Object, comptime T: type) !T {
        ...
        const parsed = try std.json.parseFromSlice(T, allocator, strValue, .{
