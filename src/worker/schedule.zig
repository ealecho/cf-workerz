...
/// Schedule handler function type - synchronous since Zig 0.11+ removed async
pub const ScheduleFn = *const fn (*ScheduledContext) void;
...
pub const ScheduledContext = struct {
    ...
    pub fn init(id: u32) !*ScheduledContext {
        const ctx = try allocator.create(ScheduledContext);
