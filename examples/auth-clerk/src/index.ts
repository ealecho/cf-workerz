/**
 * TypeScript runtime for cf-workerz with Clerk Authentication
 * 
 * This runtime extends the standard cf-workerz runtime to add:
 * - JWT verification using Clerk's Backend SDK
 * - Passing verified user claims to the Zig/WASM handler
 * - Support for both public and protected routes
 * 
...
/**
 * Clerk user claims passed to Zig after JWT verification
 */
...
interface WorkerContext {
  ...
  /** Clerk claims - populated after successful JWT verification */
 ...
 * Returns the claims if valid, or null if invalid/missing
 */
async function verifyClerkToken(
  request: Request,
  env: Env
): Promise<ClerkClaims | null> {
  // Get token from Authorization header (Bearer token) or __session cookie
  ...
  try {
    ...
    console.error('Clerk: No CLERK_SECRET_KEY or CLERK_JWT_KEY configured');
    return null;
  } catch (error) {
...
// ============================================================================
// WASM Runtime with Clerk Integration
// ============================================================================
...
class WASMRuntime {
  ...
  private buildEnvFunctions(): Record<string, unknown> {
    ...
    return {
      ...
      // ========================================================================
      // Clerk-specific functions - Access verified claims from Zig
      // ========================================================================

      /**
       * Get Clerk user ID from verified claims
       * Returns 0 if not authenticated
       */
      jsClerkGetUserId(ctxPtr: HeapPtr): number {
        const ctx = self.heap.get(ctxPtr) as WorkerContext;
        if (!ctx.clerkClaims?.userId) return 0;
        return self.putString(ctx.clerkClaims.userId);
      },

      /**
       * Get Clerk session ID from verified claims
       * Returns 0 if not authenticated
       */
      jsClerkGetSessionId(ctxPtr: HeapPtr): number {
        const ctx = self.heap.get(ctxPtr) as WorkerContext;
        if (!ctx.clerkClaims?.sessionId) return 0;
        return self.putString(ctx.clerkClaims.sessionId);
      },

      /**
       * Get Clerk organization ID from verified claims
       * Returns 0 if not in an organization
       */
      jsClerkGetOrgId(ctxPtr: HeapPtr): number {
        const ctx = self.heap.get(ctxPtr) as WorkerContext;
        if (!ctx.clerkClaims?.orgId) return 0;
        return self.putString(ctx.clerkClaims.orgId);
      },

      /**
       * Get Clerk organization role from verified claims
       * Returns 0 if not in an organization
       */
      jsClerkGetOrgRole(ctxPtr: HeapPtr): number {
        const ctx = self.heap.get(ctxPtr) as WorkerContext;
        if (!ctx.clerkClaims?.orgRole) return 0;
        return self.putString(ctx.clerkClaims.orgRole);
      },

      /**
       * Get Clerk organization slug from verified claims
       * Returns 0 if not in an organization
       */
      jsClerkGetOrgSlug(ctxPtr: HeapPtr): number {
        const ctx = self.heap.get(ctxPtr) as WorkerContext;
        if (!ctx.clerkClaims?.orgSlug) return 0;
        return self.putString(ctx.clerkClaims.orgSlug);
      },

      /**
       * Check if the request is authenticated (has valid Clerk session)
       * Returns true/false heap pointer
       */
      jsClerkIsAuthenticated(ctxPtr: HeapPtr): HeapPtr {
        const ctx = self.heap.get(ctxPtr) as WorkerContext;
        return self.heap.put(ctx.clerkClaims !== undefined);
      },
    };
  }
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Verify Clerk token before passing to WASM
    // This happens for EVERY request - Zig code can check if authenticated
    const clerkClaims = await verifyClerkToken(request, env);
    ...
    const context: WorkerContext = {
      ...
      ctx: ctx,
      resolved: false,
      clerkClaims: clerkClaims ?? undefined,
    };
